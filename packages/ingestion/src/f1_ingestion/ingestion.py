...
def ingest_raw_session_results(output_dir: Path) -> Path:
    """Write a tiny raw session results parquet file to the output directory."""
    output_dir.mkdir(parents=True, exist_ok=True)

    ingested_at = datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    records = [record.to_record() for record in _seed_session_results(ingested_at)]
    ...
    logger.info("writing raw session results", extra={"rows": table.num_rows, "path": str(output_path)})
